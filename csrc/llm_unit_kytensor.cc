...
#include <triton/client/kytensor_client.h>
...
// ============================================================================
std::shared_ptr<kc::InferInput> PrepareTensor(const std::string& name,
                              const std::vector<int64_t>& shape,
                              const std::string& datatype,
                              const uint8_t* data, size_t byte_size) {
    kc::InferInput* input = nullptr;
    kc::Error err = kc::InferInput::Create(&input, name, shape, datatype);
    if (!err.IsOk()) {
        throw std::runtime_error("failed to create InferInput: " + err.Message());
    }
    std::shared_ptr<kc::InferInput> input_ptr(input); // 确保异常安全
...
template<typename T>
std::shared_ptr<kc::InferInput> PrepareScalarTensor(const std::string& name, T value, const std::string& dtype) {
    //return PrepareTensor(name, {1}, dtype, reinterpret_cast<const uint8_t*>(&value), sizeof(T));
    kc::InferInput* input = nullptr;
    kc::Error err = kc::InferInput::Create(&input, name, {1}, dtype);
    if (!err.IsOk()) {
        throw std::runtime_error("failed to create InferInput: " + err.Message());
    }
    std::shared_ptr<kc::InferInput> input_ptr(input); // 确保异常安全
...
std::shared_ptr<kc::InferInput> PrepareStringTensor(const std::string& name, const std::string& str) {
    kc::InferInput* input = nullptr;
    kc::Error err = kc::InferInput::Create(&input, name, {1}, "BYTES");
    if (!err.IsOk()) {
        throw std::runtime_error("failed to create BYTES input: " + err.Message());
    }
    std::shared_ptr<kc::InferInput> input_ptr;
...
public:
    LlamaCppTestFixture() {
        ...
        kc::Error err = kc::KyTensorClient::Create(&grpc_client_, false);
        if (!err.IsOk()) {
        ...
        err = grpc_client_->StartStream(
            [this](kc::InferResult* result) {
                std::lock_guard<std::mutex> lock(mutex_);
                result_queue_.push(std::shared_ptr<kc::InferResult>(result));
                cv_.notify_one();
            }, false);
        if (!err.IsOk()) {
    ...
    void EnsureModelReady() {
        bool ready = false;
        kc::Error err = grpc_client_->IsModelReady(&ready, model_name_);
        if (!err.IsOk()) {
    ...
    bool SendRequest(RequestType req_type,
        ...
        std::vector<std::shared_ptr<kc::InferInput>> input_ptrs;
        std::vector<kc::InferInput*> inputs;
        //int32_t request_type = req_type;
        try {

            kc::InferInput* request_type_t;
            FAIL_IF_ERR(
                kc::InferInput::Create(&request_type_t, "request_type", {1}, "INT32"),
                "unable to create 'request_type'");
            std::shared_ptr<kc::InferInput> request_type_ptr(request_type_t);
            ...
            // input: request_id
            kc::InferInput* request_id_t;
            FAIL_IF_ERR(
                    kc::InferInput::Create(&request_id_t, "request_id", {1}, "BYTES"),
                    "unable to create 'request_id'");
            std::shared_ptr<kc::InferInput> request_id_ptr;
            ...
            // input: text_input
            kc::InferInput* text_input_t;
            FAIL_IF_ERR(
                    kc::InferInput::Create(&text_input_t, "text_input", {1}, "BYTES"),
                    "unable to create 'text_input'");
            std::shared_ptr<kc::InferInput> text_input_ptr;
            ...
            kc::InferInput* temperature_input_t;
            FAIL_IF_ERR(
                    kc::InferInput::Create(&temperature_input_t, "temperature_input", {1}, "FP32"),
                    "unable to create 'temperature_input'");
            std::shared_ptr<kc::InferInput> temperature_input_ptr(temperature_input_t);
            ...
            // input: top_k_input
            kc::InferInput* top_k_input_t;
            FAIL_IF_ERR(
                    kc::InferInput::Create(&top_k_input_t, "top_k_input", {1}, "INT32"),
                    "unable to create 'top_k_input'");
            std::shared_ptr<kc::InferInput> top_k_input_ptr(top_k_input_t);
            ...
            // input: top_p_input
            kc::InferInput* top_p_input_t;
            FAIL_IF_ERR(
                    kc::InferInput::Create(&top_p_input_t, "top_p_input", {1}, "FP32"),
                    "unable to create 'top_p_input'");
            std::shared_ptr<kc::InferInput> top_p_input_ptr(top_p_input_t);
            ...
            // input: n_keep_input
            // TODO n_keep
            kc::InferInput* n_keep_input_t;
            FAIL_IF_ERR(
                    kc::InferInput::Create(&n_keep_input_t, "n_keep_input", {1}, "INT32"),
                    "unable to create 'n_keep_input'");
            std::shared_ptr<kc::InferInput> n_keep_input_ptr(n_keep_input_t);
            ...
            kc::InferInput* n_predict_input_t;
            FAIL_IF_ERR(
                    kc::InferInput::Create(&n_predict_input_t, "n_predict_input", {1}, "INT32"),
                    "unable to create 'n_predict_input'");
            std::shared_ptr<kc::InferInput> n_predict_input_ptr(n_predict_input_t);
            ...
            kc::InferInput* cache_prompt_input_t;
            FAIL_IF_ERR(
                    kc::InferInput::Create(&cache_prompt_input_t, "cache_prompt_input", {1}, "BOOL"),
                    "unable to create 'cache_prompt_input'");
            std::shared_ptr<kc::InferInput> cache_prompt_input_ptr(cache_prompt_input_t);
            ...
            // input: stop_input "\n### Human"
            kc::InferInput* stop_input_t;
            FAIL_IF_ERR(
            kc::InferInput::Create(&stop_input_t, "stop_input", {1}, "BYTES"),
            "unable to get stop_input");
            std::shared_ptr<kc::InferInput> stop_input_ptr;
            ...
            kc::InferInput* lora_scale_input_t;
            FAIL_IF_ERR(
                    kc::InferInput::Create(&lora_scale_input_t, "lora_scale_input", {static_cast<int64_t>(lora_scale.size())}, "FP32"),
                    "unable to create 'lora_scale_input'");
            std::shared_ptr<kc::InferInput> lora_scale_input_ptr(lora_scale_input_t);
            ...
            kc::InferInput* repeat_penalty_input_t;
            FAIL_IF_ERR(
                    kc::InferInput::Create(&repeat_penalty_input_t, "repeat_penalty_input", {1}, "FP32"),
                    "unable to create 'repeat_penalty_input'");
            std::shared_ptr<kc::InferInput>repeat_penalty_input_ptr(repeat_penalty_input_t);
            ...
            kc::InferInput* stream_input_t;
            FAIL_IF_ERR(
                    kc::InferInput::Create(&stream_input_t, "stream_input", {1}, "BOOL"),
                    "unable to create 'stream_input'");
            std::shared_ptr<kc::InferInput> stream_input_ptr(stream_input_t);
        ...
        // 发起异步流式推理
        kc::InferOptions options(model_name_);
        //options.request_id_ = request_id;
        kc::Error err = grpc_client_->AsyncStreamInfer(options, inputs);
...
private:
    std::unique_ptr<kc::KyTensorClient> grpc_client_;
    std::optional<kc::Error> error_;
    std::string model_name_ = g_model_name;

    std::mutex mutex_;
    std::condition_variable cv_;
    std::queue<std::shared_ptr<kc::InferResult>> result_queue_;
};
